...
function computeTips(overview: OverviewStats | null, tools: ToolStat[]): Tip[] {
  ...
  if (!overview) {
    tips.push({
      ...
      body: "Could not fetch data from the Watchtower API. Check that the service is running.",
      severity: "high",
    })
    return tips
  }
  ...
  if (overview.total_sessions === 0) {
    tips.push({
      ...
      body: "The scout has not ingested any sessions. Verify the AGENT_DATA_DIR mount and that watchtower is running.",
      severity: "medium",
    })
    return tips
  }
