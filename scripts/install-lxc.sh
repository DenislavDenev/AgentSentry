...
validate_mount() {
  ...
  if [[ -d "$AGENT_DATA_DIR/projects" ]]; then
    ...
    info "Warning: no projects/ subdirectory found. AgentSentry will watch once Claude Code creates session files."
  fi
}
...
# ---------------------------------------------------------------------------
# STEP 6 — Python workspace (watchtower only)
# ---------------------------------------------------------------------------
install_python() {
...
# ---------------------------------------------------------------------------
# STEP 8 — Data directories and SQLite location
# ---------------------------------------------------------------------------
create_dirs() {
  mkdir -p "$STATE_DIR"
...
# ---------------------------------------------------------------------------
# STEP 9 — Systemd unit (single service)
# ---------------------------------------------------------------------------
write_systemd_unit() {
